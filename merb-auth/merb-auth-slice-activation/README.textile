...
1. Do you respond_to?(:active?)
  2. If yes, check if active and return the user if they are
  3. If no,  return the user (i.e. no activation check)
...
Use the standard slice configuration hash to set these up
@Merb::Slices::config[:merb_auth_slice_activation]@
...
<pre><code>
  $ rake slices:merb_auth_slice_activation:stubs
</code></pre>
...
3. The default layout, as specified in Merb::Slices::config[:merb_auth_slice_activation][:layout]
...
7. Public assets you (optionally) install using rake slices:merb_auth_slice_activation:install
...
9. Stubs of classes/views/files for the end-user to override - usually these 
   mimic the files in app/ and/or public/; use rake slices:merb_auth_slice_activation:stubs to
   get started with the override stubs. Also, slices:merb_auth_slice_activation:patch will
...
rake -T slices:merb_auth_slice_activation
...
Merb::BootLoader.before_app_loads do
  ...
  Merb::Slices::config[:merb_auth_slice_activation][:option] = value
  
end

file: config/router.rb

# example: /merb_auth_slice_activation/:controller/:action/:id
...
By default :merb_auth_slice_activation is used. If you need to override
stylesheets or javascripts, just specify your own files in your layout
instead/in addition to the ones supplied (if any) in 
host-app/public/slices/merb-auth-slice-activation.

In any case don't edit those files directly as they may be clobbered any time
rake merb_auth_slice_activation:install is run.
